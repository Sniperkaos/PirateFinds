...
public class SealItem extends CommandConsumer<CommandSender> {

	public SealItem() {
		this.setPermission("pf.commands.sealitem");
	}
	...
	protected void execute(CommandSender player, ArrayList<String> args) {
		...
		if(expansion.isLoaded()) {
			Unseal.sealItem((Player) player, ((Player) player).getInventory().getItemInMainHand());
