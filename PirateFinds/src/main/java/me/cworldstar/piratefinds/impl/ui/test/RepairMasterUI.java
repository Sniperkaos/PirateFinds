...
public class RepairMasterUI extends BaseUIObject {
	...
	private static ItemStack odd_glass = new ItemStack(Material.BLACK_STAINED_GLASS_PANE);
	...
	static {
		...
		HeadMeta.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&f&lREPAIR"));
	...
	public RepairMasterUI(Player p) {
		super(p, InventorySize.MEDIUM, "Repair Master");
	}
	...
	protected void decorate(Inventory i) {
		...
		this.addTicker(new MenuHandler<TickerTickEvent>((TickerTickEvent e) -> {
			barrier_slots.forEach((Integer slot) -> {
				if(slot % 2 == 0) {
					if(even) {
						this.setItem(slot, even_glass);
					} else {
						this.setItem(slot, odd_glass);
					}
		...
		this.addUnclickableItem(12, new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
		this.addUnclickableItem(14, new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
		...
		this.addMenuClickHandler(22, new MenuHandler<InventoryClickEvent>((InventoryClickEvent e) -> {
			...
			if(e.getClickedInventory().getItem(13) == null) {
					p.sendMessage(ChatUtils.apply("&7[ &f&lRepair Master &7]: You didn't give me an item."));
					p.playSound(p, Sound.ENTITY_VILLAGER_NO, 0.8F, 0.4F);
				return;
			}
			
			if(price == 0) {
				p.sendMessage("RepairMaster Error: Price of item is 0, this is impossible! Try re-inserting the item without shift-clicking.");
				return;
			}
			
			if(p.getLevel() < price) {
				p.sendMessage(ChatUtils.apply("&7[ &f&lRepair Master &7]: You do not have enough experience levels."));
				p.playSound(p, Sound.ENTITY_VILLAGER_NO, 0.8F, 0.4F);
				return;
			}
			
			if(e.getClickedInventory().getItem(13).getItemMeta() == null) {
				p.sendMessage(ChatUtils.apply("&7[ &f&lRepair Master &7]: This item does not have item meta."));
				p.playSound(p, Sound.ENTITY_VILLAGER_NO, 0.8F, 0.4F);
				return;
			}
			
			if(!(e.getClickedInventory().getItem(13).getItemMeta() instanceof Damageable)) {
				p.sendMessage(ChatUtils.apply("&7[ &f&lRepair Master &7]: I am not able to repair this."));
				p.playSound(p, Sound.ENTITY_VILLAGER_NO, 0.8F, 0.4F);
				return;
			}
			
			int exp_to_subtract = ExperienceUtils.getExpAtLevel(price);
			ExperienceUtils.changePlayerExp(p, -exp_to_subtract);
