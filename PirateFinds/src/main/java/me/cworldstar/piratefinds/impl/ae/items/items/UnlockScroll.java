...
public class UnlockScroll extends AbstractPFItem {
	...
	static {
		...
		meta.setItemName(ColorUtils.format("&7&l(&c&l!&7&l) &fUnlock Scroll"));
		meta.setLore(List.of(new String[] {
				ColorUtils.format("&c&oDragging and dropping this onto an item"),
				ColorUtils.format("&c&owill permanently unlock the item."),
		}));
	...
	public boolean checkExpend(Player p, ItemStack on) {
		...
		if(!lore.contains(Locked.LOCKED_LORE_LINE)) {
			p.sendMessage(ChatUtils.createBroadcast("&7This item is not locked. Your scroll will not be expended."));
			return false;
		}
