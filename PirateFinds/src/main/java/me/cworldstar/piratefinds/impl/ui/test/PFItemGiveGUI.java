...
public class PFItemGiveGUI extends PagedUIObject {
	...
	public void decorateLayout(Inventory i) {
		...
		for(Entry<String, AbstractPFItem> items : PFItemClass.getItems().entrySet()) {
			...
			slot = layout1.firstEmpty();
			layout1.addUnclickableItem(slot, items.getValue().getPFItem());
			layout1.addMenuClickHandler(slot, new MenuHandler<InventoryClickEvent>((InventoryClickEvent e) -> {
				...
				InventoryUtils.addOrDropItem(this.getOwner().getInventory(), items.getValue().build());
			}));
		}
	}
